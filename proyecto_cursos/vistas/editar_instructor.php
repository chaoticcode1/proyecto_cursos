...
if (isset($_GET['id'])) {
    ...
    // Obtener los datos del instructor a editar
    try {
...
if (isset($_POST['submit'])) {
    ...
    try {
        $sql = "UPDATE instructores SET nombre = :nombre, apellido = :apellido, cedula = :cedula, correo = :correo, telefono = :telefono, fecha_nacimiento = :fecha_nacimiento, edad = :edad WHERE id = :id";
        $stmt = $conn->prepare($sql);
        ...
        // Mostrar mensaje de éxito y redirigir a mostrar_instructores.php
        echo "<script>
        ...
        <div class="form-container mt-5">
            ...
            <form method="POST" action="" onsubmit="return validarCorreo()">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nombre" name="nombre" value="<?php echo $instructor['nombre']; ?>" required
                        onkeypress="validarLetrasConO(event)">
                </div>
                <div class="mb-3">
                    <label for="apellido" class="form-label">Apellido</label>
                    <input type="text" class="form-control" id="apellido" name="apellido" value="<?php echo $instructor['apellido']; ?>" required
                        onkeypress="validarLetrasConO(event)">
                </div>
                <div class="mb-3">
                    <label for="cedula" class="form-label">Cédula</label>
                    <input type="text" class="form-control" id="cedula" name="cedula" value="<?php echo $instructor['cedula']; ?>" required 
                           onkeypress="validarCedula(event)" maxlength="8" minlength="7">
                </div>
                <div class="mb-3">
                    <label for="correo" class="form-label">Correo</label>
                    <input type="email" class="form-control" id="correo" name="correo" value="<?php echo $instructor['correo']; ?>" required
                           onblur="validarCorreo()">
                </div>
                <div class="mb-3">
                    <label for="telefono" class="form-label">Teléfono</label>
                    <input type="text" class="form-control" id="telefono" name="telefono" value="<?php echo $instructor['telefono']; ?>" 
                           onkeypress="validarTelefono(event)" maxlength="11" minlength="11">
                </div>
                <div class="mb-3">
                    <label for="fecha_nacimiento" class="form-label">Fecha de Nacimiento</label>
                    <input type="date" class="form-control" id="fecha_nacimiento" name="fecha_nacimiento" value="<?php echo $instructor['fecha_nacimiento']; ?>" required>
                </div>
